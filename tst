...
app.layout = html.Div([
        html.H2('Vessel fuel consumption per Nautical mile prediction (kg/nm)',
              style={'margin-left': '500px', 'margin-bottom': '50px'}),
        html.Div([
        ...
        html.Div([
          ...
          dbc.Input(
              id='deadweight',
              min=min_dwt, max=max_dwt, #step=1000,
          ...
          style={'width': '75%', 'display': 'flow-root'}),
        html.Hr(),
        html.Div([
        ...
        html.Br(),
        html.Table([
            html.Tr([html.Td(['Linear Model projection of kg of fuel per nautical mile: ']), html.Td(id='linear')]),
            html.Tr([html.Td(['Neural Net projection of kg of fuel per nautical mile: ']), html.Td(id='neural')]),
        ]),
        html.Hr(),
        dcc.Graph(id='graph')
        ]
#+ [html.Div(id='all-inputs')]
...
def callback_models(nclicks, yearbuilt, dwt, enginekw, grt):
    ...
    return ({'data': data_points_for_graph,
            'layout': go.Layout(title='Fuel consumption per nautical mile on same yearbuilt Vessels',
                                xaxis={'title':'Deadweight'},
                                yaxis={'title':'kg of fuel per nautical mile '})}, consumption, consumption2)
